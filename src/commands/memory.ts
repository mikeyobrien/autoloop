...
export function dispatchMemory(args: string[]): boolean {
  ...
  switch (sub) {
    ...
    case "status":
      ...
      return true;
    case "find":
      if (!args[1] || args[1] === "--help") { console.log("Usage: autoloops-ts memory find <pattern...>"); return true; }
    ...
    case "add":
      ...
      return true;
    case "remove":
      if (!args[1] || args[1] === "--help") { console.log("Usage: autoloops-ts memory remove <id> [reason...]"); return true; }
...
function dispatchMemoryAdd(args: string[]): void {
  ...
  switch (kind) {
    case "learning":
      if (!args[1] || args[1] === "--help") { console.log("Usage: autoloops-ts memory add learning <text...>"); return; }
      memory.addLearning(resolveRuntimeProjectDir(), args.slice(1).join(" "), "manual");
      return;
    case "preference":
      if (!args[1] || args[1] === "--help") { console.log("Usage: autoloops-ts memory add preference <category> <text...>"); return; }
      memory.addPreference(resolveRuntimeProjectDir(), args[1], args.slice(2).join(" "));
      return;
    case "meta":
      if (!args[1] || args[1] === "--help") { console.log("Usage: autoloops-ts memory add meta <key> <value...>"); return; }
