# autoloops-ts as Control Plane

autoloops-ts is the execution engine and state model for long-horizon autonomous work. External interfaces — CLI, chat, cron, future API/UI — are thin shells that launch, observe, and report on runs. They do not orchestrate.

## Architectural roles

### Control plane (autoloops-ts)
...
| **Chat** | Thin intake: accept objectives, dispatch to autoloops-ts, report results |
...
## When to use autoloops-ts

Use autoloops-ts when the task is:
...
- autoloops-ts should not become a kitchen sink for every automated task
...
3. **Shells are thin.** CLI, chat, and cron dispatch to autoloops-ts — they do not contain loop logic.
