...
| `event_loop.completion_must_be_last` | bool | `false` | Ralph-parity ordering policy (opt-in). When `true`, a completion claim via `completion_event` is rejected if any other event was emitted after it within the same turn — set-membership alone (the default) is not enough. Default `false` preserves the order-insensitive check: once the completion event and all `required_events` have appeared anywhere in the run, completion is accepted regardless of intra-turn ordering. |
| `event_loop.audit_file_mods` | bool | `false` | Ralph-parity emit-boundary audit (opt-in). After every iteration, diffs the working tree against `HEAD`; if the single acting role (topology's `[[role]]` for the turn) declares `disallowed_tools` or `read_only = true` and files changed, journals and emits a `policy.file_modification_violation` event with the role and file list. Purely observational — never alters loop control flow on its own; parent orchestrators react to the emitted event. No-ops outside a git work tree or when the acting role is ambiguous. |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `backend.kind` | string | `""` (auto) | Backend type. Valid non-empty kinds are `"command"`, `"pi"`, `"claude-sdk"`, `"acp"`, `"kiro"`, and `"hermes"`. The `"kiro"` and `"hermes"` aliases normalize to `kind = "acp"` with the matching provider. Any other non-empty value is a configuration error. |
...
**Backend process hardening.** Every backend mode (Claude SDK, Pi RPC, ACP, and shell command), role override, parallel branch, and metareview inherits a sanitized environment. Autoloop preserves ordinary project/toolchain variables and provider credentials, but removes process-injection hooks (`NODE_OPTIONS`, `PYTHONPATH`, `LD_PRELOAD`, `DYLD_*`, language runtime option hooks), Git authority overrides (`GIT_CONFIG_*`, `GIT_DIR`, `GIT_WORK_TREE`, askpass/SSH command hooks), relative or project-owned `PATH` entries, and TLS key logging. Proxy variables are accepted only as credential-free proxy endpoints; malformed or userinfo-bearing proxy URLs fail the run before backend startup.

Configured file prompts (`event_loop.prompt_file`, `review.prompt_file`, `harness.instructions_file`, and topology role `prompt_file`) must resolve inside their declared project/preset directory after symlink resolution. Parent traversal and symlink escapes fail closed rather than injecting host files into model context.
