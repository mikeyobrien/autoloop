...
- **Completion and required-event semantics**: Behavior of `completion.event`, `completion.requiredEvents`, `completion.must_be_last`, and how the harness decides loop termination. Live harness completion/routing trusts only parent-accepted agent emits (request-queue ingress + `authority_id`). Resume rehydrates acceptance from a MAC'd harness-owned ledger stored outside `AUTOLOOP_STATE_DIR` (under `XDG_RUNTIME_DIR` or the process temp root), never from the backend-writable journal. Same-user unrestricted filesystem adversaries remain out of scope for durable cryptographic separation.
