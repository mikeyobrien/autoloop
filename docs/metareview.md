...
The metareview is a meta-level review pass that runs periodically between normal loop iterations. Its job is to improve loop hygiene — consolidating stale context, trimming noisy working files, and storing durable learnings — without directly advancing the task. In addition to raw memory and scratchpad state, the review prompt also carries a small **context pressure** summary so the metareview can react to memory bloat and routing instability directly.
...
|-----|---------|-------------|
| `review.enabled` | `true` | Enable or disable the metareview. Parsed as a truthy string (`true`, `1`, `yes`). |
...
| `review.prompt_file` | `metareview.md` | Path (relative to project dir) to a file containing the review prompt. Only read when `review.prompt` is empty. |
...
2. Contents of the file at `review.prompt_file` (default: `metareview.md` in the project directory).
3. Empty string — if neither is set and the file does not exist, no additional instructions are injected.

When a prompt file or inline text is present, it appears in the rendered review prompt under the heading **"Additional metareview instructions:"**.

## The `metareview.md` file

The default review prompt file is `metareview.md` at the root of the project directory. A typical metareview prompt looks like:
...
Prefer bounded hygiene edits to `autoloops.toml`, `topology.toml`, `harness.md`, `metareview.md`, `roles/*.md`, `.autoloop/context.md`, `.autoloop/plan.md`, `.autoloop/progress.md`, `.autoloop/logs/`, and `.autoloop/docs/*.md`.
...
- A role statement: *"You are the metareview meta-reviewer for this loop."*
- A bounded-permissions instruction allowing hygiene edits to runtime-facing loop files such as `autoloops.toml`, `topology.toml`, `harness.md`, `metareview.md`, `roles/*.md`, `.autoloop/context.md`, `.autoloop/plan.md`, `.autoloop/progress.md`, `.autoloop/logs/`, and `.autoloop/docs/*.md`.
...
- Guidance to use `autoloop memory add ...` for short durable lessons or operator notes that should persist across turns.
...
- Useful `autoloop inspect` commands for the latest iteration.
...
- Review output may still include bounded hygiene edits to runtime-facing loop files plus `autoloop memory add ...` commands for durable notes.
