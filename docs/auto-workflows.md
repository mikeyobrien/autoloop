...
Autoloops ships a family of `auto*` preset workflows. Each one is a self-contained agentic loop with a distinct behavioral center, topology, and shared-state contract. This document is the canonical taxonomy.

Presets are the primary product surface of the autoloops-ts control plane. New workflows are added as preset directories — not as code changes to the runtime. See [Platform Architecture](platform.md) for how presets fit into the broader system.
