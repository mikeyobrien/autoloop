...
- When a change affects a user/operator-facing CLI or helper tool, smoke that surface directly (for example `./.autoloop/autoloops ...`) instead of relying only on a lower-level `tonic run ...` wrapper that may hide the real shell semantics.
...
- Emit only from the allowed next-event set shown in the prompt. This loop stays on the normal workflow events unless topology/prompt explicitly allows extra side-channel emits; until then, keep `slice.*`, `issue.*`, and `context.archived` facts in `.autoloop/progress.md`.
...
- **Markdown** = curated intent (`.autoloop/context.md`, `.autoloop/plan.md`, concise `.autoloop/progress.md` summary)
- **Docs** = archived reference material (`.autoloop/docs/*.md`)
- **Memory** = durable lessons, preferences, meta notes

Until topology explicitly supports additional coordination events, record slice start/verification/commit facts and issue dispositions in `.autoloop/progress.md`, commit messages, and review/finalizer handoffs rather than separate `slice.*` or `issue.*` emits.
