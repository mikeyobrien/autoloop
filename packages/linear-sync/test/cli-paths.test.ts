...
const mocks = vi.hoisted(() => ({
  ...
  LinearAdapter: vi.fn(function LinearAdapter() {}),
}));

vi.mock("@mobrienv/autoloop-issue-sync-core", () => ({
  createJsonlTasksApi: mocks.createJsonlTasksApi,
  pull: mocks.pull,
  push: mocks.push,
  release: mocks.release,
  resolveIssueSyncPaths: mocks.resolveIssueSyncPaths,
}));
vi.mock("../src/adapter.js", () => ({ LinearAdapter: mocks.LinearAdapter }));
...
describe("linear-sync CLI resolved paths", () => {
  it("uses resolver-provided config, tasks, and state paths for every command", async () => {
    const projectDir = mkdtempSync(join(tmpdir(), "linear-sync-cli-paths-"));
    ...
    try {
      process.argv = ["node", "autoloop-linear-sync", "pull"];
      ...
      expect(mocks.release.mock.calls[0][3]).toBe("v2.0.0");
      expect(mocks.release.mock.calls[0][4]).toBe("repo");
      expect(mocks.release.mock.calls[0][6]).toEqual({ archive: false });
      expect(log).toHaveBeenCalledWith(
        "autoloop-linear-sync release: promoted 0 issue(s) to Done",
      );
    } finally {
