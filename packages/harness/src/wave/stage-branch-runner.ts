...
export function buildStageBranchRunner(
  ...
  return async (spec: StageSpec): Promise<FanoutBranchResult> => {
    ...
    // A stage branch is a single unit of work: wait for it to finish (the
    // per-branch wall-clock cap is enforced by the supervisor). `wait_for_all`
    // preserves the original unconditional-wait semantics after the
    // aggregate-mode signature change to `joinParallelBranches`.
    const { results } = joinParallelBranches(
      loop,
      iter,
      stageId,
      [launched],
      { mode: "wait_for_all", timeoutMs: loop.stage?.branchTimeoutMs ?? 0 },
      launched.launchMs,
    );
