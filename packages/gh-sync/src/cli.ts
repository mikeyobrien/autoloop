...
function loadIssueSyncConfig(tomlPath: string): IssueSyncConfig {
...
async function main() {
  ...
  const config = loadIssueSyncConfig(paths.configFile);
  ...
  if (subcommand === "pull") {
    const result = await pull(adapter, config, tasksApi, paths.stateFile);
