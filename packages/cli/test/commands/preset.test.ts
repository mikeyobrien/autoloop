...
import { initSingleFilePreset } from "../../src/commands/init.js";
...
describe("dispatchPreset promote", () => {
  ...
  it("promotes a preset generated by `autoloop init --single-file`", () => {
    const src = join(work, "generated.toml");
    initSingleFilePreset(src);
    const out = captureOut(() => dispatchPreset(["promote", src, "generated"]));
    expect(out).toContain("promoted");
    expect(
      existsSync(
        join(
