...
- **Desired stdlib support:** Native compiled binaries should support the string host/runtime surface needed for normal autoloops CLI execution, including real run-path smoke tests
- **Status:** Open; reproduced on 2026-03-29 with `scripts/compiled-run-check.sh /tmp/autoloops-release-bin` (exit 2, `error: host error: unknown host function: str_length`)
