...
const TEST_ROLES: Role[] = [
  { id: "planner", prompt: "You are the planner.", promptFile: "", emits: ["tasks.ready"] },
  { id: "builder", prompt: "You are the builder.", promptFile: "", emits: ["review.ready"] },
  { id: "critic", prompt: "You are the critic.", promptFile: "", emits: ["review.passed"] },
];

describe("parseProfileSpec", () => {
  it("parses repo:name", () => {
    expect(parseProfileSpec("repo:phoenix")).toEqual({ scope: "repo", name: "phoenix" });
  });

  it("parses user:name", () => {
    expect(parseProfileSpec("user:strict-review")).toEqual({ scope: "user", name: "strict-review" });
  });

  it("throws on bare name without scope", () => {
    expect(() => parseProfileSpec("phoenix")).toThrow('must be "repo:<name>" or "user:<name>"');
  });

  it("throws on invalid scope", () => {
    expect(() => parseProfileSpec("global:phoenix")).toThrow('must be "repo" or "user"');
  });
...
describe("resolveProfileFragments", () => {
  it("reads fragment from correct path", () => {
    ...
    const profileDir = join(workDir, ".autoloop", "profiles", "phoenix", "mypreset");
    mkdirSync(profileDir, { recursive: true });
    writeFileSync(join(profileDir, "planner.md"), "Extra planner guidance.");

    const result = resolveProfileFragments(["repo:phoenix"], "mypreset", TEST_ROLES, workDir);
    expect(result.fragments.get("planner")).toContain("Extra planner guidance.");
  ...
  it("composes multiple profiles in order", () => {
    ...
    const result = resolveProfileFragments(
      ["repo:alpha", "repo:beta"], "preset1", TEST_ROLES, workDir,
    );
    const fragment = result.fragments.get("builder") ?? "";
    expect(fragment.indexOf("Alpha guidance.")).toBeLessThan(fragment.indexOf("Beta guidance."));
  });

  it("throws on missing profile directory", () => {
    const workDir = tmpDir("missing-profile");
    expect(() =>
      resolveProfileFragments(["repo:nonexistent"], "preset1", TEST_ROLES, workDir),
    ).toThrow("does not exist");
  });

  it("warns on fragment for unknown role", () => {
    const workDir = tmpDir("unknown-role");
    const profileDir = join(workDir, ".autoloop", "profiles", "test", "preset1");
    mkdirSync(profileDir, { recursive: true });
    writeFileSync(join(profileDir, "nonexistent-role.md"), "Ignored.");

    const result = resolveProfileFragments(["repo:test"], "preset1", TEST_ROLES, workDir);
    expect(result.warnings.some((w) => w.includes("unknown role"))).toBe(true);
    expect(result.warnings.some((w) => w.includes("contributed no fragments"))).toBe(true);
  });
  ...
  it("warns when profile has no matching fragments for preset", () => {
    ...
    const result = resolveProfileFragments(["repo:empty-profile"], "preset1", TEST_ROLES, workDir);
...
describe("applyProfileFragments", () => {
  it("appends fragment to matching role prompt", () => {
    ...
    expect(updated.find((r) => r.id === "builder")?.prompt).toBe("You are the builder.\nExtra builder text.");
    // Other roles unchanged
    expect(updated.find((r) => r.id === "planner")?.prompt).toBe("You are the planner.");
  });
...
describe("--no-default-profiles suppression", () => {
  it("config defaults are read by getProfileDefaults", async () => {
    ...
    const { getProfileDefaults, loadProject } = await import("../src/config.js");
