...
On every activation:
- Read `context.md`, `plan.md`, and `progress.md` if they exist.
...
On first activation:
- Create or refresh:
  - `context.md` — request summary, source type, constraints, repo patterns, acceptance criteria
  - `plan.md` — numbered high-level steps, not a blob checklist
  - `progress.md` — current step, active slice, verification notes, completed steps
  - `logs/` directory if useful
