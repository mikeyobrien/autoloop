...
- Treat the active slice description and likely-to-change files as a file budget. If you cannot explain every touched file as necessary for that slice, shrink the change or record the justification in `.autoloop/progress.md` before handoff.
...
- If the latest rejection or handoff changed the active issue/slice, refresh `.autoloop/context.md` and `.autoloop/plan.md` so they describe the current objective before coding; archive resolved detours under `.autoloop/docs/` instead of leaving stale focus text in the active files.
- Re-check the routing context in your prompt and emit only an allowed workflow event (`review.ready` or `build.blocked`).
- Update `.autoloop/progress.md` with the slice start, what you verified, the slice commit hash, and any relevant issue dispositions.

Process:
1. Understand the active slice and its acceptance criteria.
2. Record the slice start in `.autoloop/progress.md` before editing.
...
8. Record concise verification evidence in `.autoloop/progress.md` and longer output in `.autoloop/logs/` when useful.
9. Before commit, inspect the exact file set that will land in the slice commit and remove unrelated churn. If an extra file is truly required, record why that file is part of the slice in `.autoloop/progress.md`.
10. If the slice changes prompt composition, prompt injection, or routing advisories, inspect the rendered prompt/artifacts directly (for example `./.autoloop/autoloops inspect prompt <iteration> --format md` or fixture-local branch artifacts) and record the proof path in `.autoloop/progress.md`; tests alone are not enough.
11. Commit the completed slice before handoff. Each completed slice should land as its own commit.
12. Record the commit hash in `.autoloop/progress.md`.
13. If you discover a relevant issue, record it in the `Relevant Issues` section in `.autoloop/progress.md` with an explicit disposition (`fix-now`, `fix-next`, `deferred`, or `out-of-scope`).
