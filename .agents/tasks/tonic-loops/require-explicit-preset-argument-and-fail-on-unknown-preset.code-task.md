...
## Background
Today `src/main.tn` parses `autoloops run [project-dir] [prompt...]` using `looks_like_project_dir/1`. If the first positional argument is not an existing directory with `autoloops.toml` (or `autoloops.conf`), it is treated as prompt text and the project directory falls back to `.`.

In this repo, `.` is the root autocode preset, so a command like:

```bash
autoloops run autoqa "qa recent changes"
```
...
- `autoloops.toml`
- `examples/autocode/autoloops.toml`
- `examples/autoqa/autoloops.toml`
...
5. Keep support for explicit preset directories/paths that actually exist and contain autoloops config.
...
1. **Missing Preset Fails Early**
   - Given a user runs `autoloops run` without a preset/path argument
...
2. **Unknown Preset Does Not Fall Back To Autocode**
   - Given a user runs `autoloops run autoqaa "qa recent changes"`
...
4. **Valid Explicit Preset Path Still Works**
   - Given a user runs `autoloops run examples/autoqa "qa recent changes"`
...
5. **Help Text Matches The New Contract**
   - Given a user runs `autoloops --help`
...
## Metadata
- **Complexity**: Medium
- **Labels**: autoloops, cli, presets, fail-closed, autocode, autoqa, argument-parsing
