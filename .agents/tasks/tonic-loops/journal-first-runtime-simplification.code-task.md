...
## Description
Push more autoloops runtime coordination state into the append-only JSONL journal while preserving editable markdown for curated intent. The goal is to make the journal the canonical source of truth for machine-owned runtime facts such as issue tracking, slice lifecycle, verification status, and commit-per-slice history, while reducing `.autoloop/progress.md` to a lightweight human-facing summary rather than the primary coordination store.

## Background
Autoloops already uses a journal-first runtime model for core loop events such as `loop.start`, `iteration.start`, `backend.start`, `backend.finish`, `iteration.finish`, and `loop.complete`. That part is working well.
...
Today these are often written into `.autoloop/progress.md` or implied by prompts, which makes them harder to inspect, enforce, and project consistently. The next step is to move more of this machine-owned coordination state into structured journal events while keeping authored markdown files for current plan/context and archived docs.

The intended architecture is:
- **Journal** = append-only runtime truth
- **Markdown working files** = current curated intent (`.autoloop/context.md`, `.autoloop/plan.md`, concise `.autoloop/progress.md`)
...
6. Update projections or summary surfaces so `.autoloop/progress.md` can become a lightweight summary rather than the only source of truth for issue/slice tracking.
7. Preserve `.autoloop/plan.md` and `.autoloop/context.md` as curated editable files rather than forcing them into pure event sourcing.
...
- Existing working-file contracts (`.autoloop/context.md`, `.autoloop/plan.md`, `.autoloop/progress.md`)
...
5. Reduce `.autoloop/progress.md` toward a concise operator-facing summary while preserving its usefulness for humans.
...
1. **Relevant Issue Lifecycle Is Structured**
   ...
   - Then the journal records structured issue lifecycle events with explicit disposition and owner instead of relying only on prose in `.autoloop/progress.md`
...
5. **Progress Summary Is No Longer The Sole Coordination Store**
   ...
   - Then the canonical machine-owned facts come from the journal, while `.autoloop/progress.md` can remain a concise summary rather than the only source of truth

6. **Curated Markdown Stays Curated**
   - Given `.autoloop/plan.md`, `.autoloop/context.md`, and `docs/*.md`
...
## Metadata
- **Complexity**: High
- **Labels**: autoloops, journal, runtime-state, event-sourcing, projections, hyperagent, autocode
