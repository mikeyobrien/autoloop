...
On every activation:
- Read `.miniloop/review-context.md`, `.miniloop/review-findings.md`, and `.miniloop/progress.md` if they exist.
...
- Create or refresh:
  - `.miniloop/review-context.md` — diff summary, affected files, what each change does, architectural context.
  - `.miniloop/progress.md` — review phase, files to check.
...
- Build a risk map in `.miniloop/review-context.md`: changed files, adjacent dependencies, trust boundaries, invariants that could break, and missing tests or context.
- Note any files that the diff touches which interact with other systems — the checker needs this.
- If the diff is large, break it into logical units (e.g., per-file or per-feature) in `.miniloop/review-context.md`.
