...
On every activation:
- Read `.autoloop/pr-context.md`, `.autoloop/pr-draft.md`, `.autoloop/pr-result.md`, and `.autoloop/progress.md`.
...
2. Use the validated title/body from `.autoloop/pr-draft.md`.
3. Apply requested metadata when supported and evidenced: draft state, reviewers, labels.
4. Respect the normalized mode from `.autoloop/pr-context.md`:
...
5. Write `.autoloop/pr-result.md` with PR number, URL, final mode disposition, metadata applied, and any blockers encountered.
6. Update `.autoloop/progress.md` with exact commands run and final state.
...
- Never emit `task.complete` without recording the PR URL or equivalent durable result in `.autoloop/pr-result.md`.
