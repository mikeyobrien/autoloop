...
Global rules:
- Shared working files are the source of truth: `.miniloop/bug-report.md`, `.miniloop/fix-log.md`, `.miniloop/progress.md`.
- Preserve canonical filenames exactly as they exist on disk. If the upstream report is `qa-report.md`, keep that spelling/path everywhere; never invent `qa_report.md` or move it under `.miniloop/` unless the file really lives there.
...
- Record exact commands and key outputs in `.miniloop/progress.md` or `.miniloop/fix-log.md`, not just summaries.
...
State files:
- `.miniloop/bug-report.md` — the original bug report, reproduction steps, and root cause analysis.
- `.miniloop/fix-log.md` — log of fixes applied: what was changed, why, verification results.
- `.miniloop/progress.md` — current bug being fixed, what the next role should do.