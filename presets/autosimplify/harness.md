...
Global rules:
- Shared working files are the source of truth: `simplify-context.md`, `simplify-plan.md`, and `progress.md`.
...
- The simplifier must record exact files changed and exact verification command(s) with outputs in `progress.md` before claiming success.
...
State files:
- `simplify-context.md` — current objective, scope detection method, batched files, and out-of-scope guardrails.
- `simplify-plan.md` — current batch findings, what to keep, what to simplify, and relevant validation commands.
- `progress.md` — active batch, edits made, verification evidence, rejected attempts, and next action.
