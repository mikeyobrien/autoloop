...
Global rules:
- Shared working files are the source of truth: `.miniloop/perf-profile.md`, `.miniloop/perf-log.jsonl`, `.miniloop/progress.md`.
...
State files:
- `.miniloop/perf-profile.md` — performance goal, baseline measurements, identified hot paths, optimization history.
- `.miniloop/perf-log.jsonl` — append-only log. Each line: `{"id":N, "target":"...", "change":"...", "metric_before":..., "metric_after":..., "verdict":"keep|discard", "reason":"..."}`.
- `.miniloop/progress.md` — current optimization target, what the next role should do.